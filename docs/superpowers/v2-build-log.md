...
- **2026-06-11 — Phase 4: the archipelago (organic world) landed.** The headline transformation: rooms-in-a-row → ONE pannable world of organic islands on a dark sea. New pure module `layout/terrain.ts`: islands are **star-convex noise blobs** — a cell is land iff `dist(cell, anchor) < R · coastFactor(angle, seed)` where coastFactor is two-octave periodic value noise over the *angle only* (14- and 31-point lattices on the existing `det()` hash). Angle-only noise was the key simplification vs. the review doc's 2D-noise sketch: it guarantees the blob is *connected, hole-free, and strictly monotonic in R* (growth only ever adds coastline) with zero flood-fill machinery — three stability invariants for the price of one design choice, all pinned by tests. Edges render by **quadrant marching-squares**: each coast cell assembles from four 8×8 quadrants of the Ninja field tileset's 5×3 autotile blocks (3×3 blob + 2×2 inner corners — decoded the inner-corner notches sit at the 2×2's outer corners), which covers every neighbor case with no new art. Themes are *terrain + structures*: office = scholar's camp (tent landmark from `tileset_camp.png`, rug+desk stations) on a dark-green grove, mine = dig site (timbered cave-mouth landmark, ore faces) on orange earth, farm = fenced crop plots + shade-tree landmark on light green. **Placement:** golden-angle spiral (`θ=k·2.4, r=11√k`, 0.82 vertical squash) takes the first point ≥32 tiles from every earlier anchor — region i depends only on regions 0..i-1, so adding a folder never moves existing islands. Stations place greedily on the blob *as it was when that session arrived* (radius R(i+1), considering only earlier rects) — append-stable like Phase 3, now on organic ground; a precomputed per-region `normDist` field (Float32Array) makes every membership test an array lookup. Lounge = south coast on the 1-session blob (never moves); landmark = fixed rect north of the anchor (always inside the minimum blob). **Paths:** each new island links to its nearest older island with a det-wobbled Bresenham causeway of procedural stepping stones, skipping land cells; signpost at each gateway, arrivals/departures walk via the gateway instead of dropping from y=-2. **Perf:** `Skin.drawRoom` split into `drawRegionBase` (static: ground autotile, shallows rim, landmark, props, scatter — rendered ONCE per island into an offscreen canvas keyed by dirKey+radius+stations, blitted per frame, viewport-culled) and `drawRegionLive` (swaying plants, growing crops, lamp flicker). Sea is O(visible): coarse-grid det glints + wave dashes. Camera: auto-fit handles negative world origins and fits below scale 1 (1/8 steps, floor 0.3 — a 5-island world fits a 900×620 window); `?zoom=N&focus=i` debug/share param jumps to an island. Demo grew two late folders (`-demo-cli` farm, `-demo-docs` mine) so `?demo=31` shows a 5-island archipelago; `?demo=14` unchanged. Soft edges accepted: island radius pops (no growth lerp animation yet), paths can re-route ±1 tile when a coast grows, a long-lived world is sparser than a packed one (stability > compactness, per the review doc). 97 tests green (terrain invariants: monotonic growth, connectivity, quadrant tables; layout invariants: append-only anchors/stations, no island overlap, stones never on land), tsc strict + vite green, verified across 5 screenshot rounds at `?demo=14/31`, zoom 0.375–2.5, all three themes close-up.
